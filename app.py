...
class Assignment3VPN:
    ...
    def __init__(self, master=None):
        ...
        self.connectButton = builder.get_object(
            'connectButton', self.mainwindow)
        self.secureButton = builder.get_object('secureButton', self.mainwindow)
        self.clientRadioButton = builder.get_object(
            'clientRadioButton', self.mainwindow)
        self.serverRadioButton = builder.get_object(
            'serverRadioButton', self.mainwindow)
        ...
        builder.import_variables(
            self, ['mode', 'hostName', 'port', 'sharedSecret', 'textMessage'])
        ...
        self.server_thread = Thread(
            target=self._AcceptConnections, daemon=True)
    ...
    def _CreateTCPConnection(self):
        ...
        try:
            ...
            if self.mode.get() == 0:
                ...
                self._AppendLog(
                    "CLIENT: Connection established successfully. You can now send/receive messages.")
            else:
    ...
    def _AcceptConnections(self):
        try:
            ...
            self._AppendLog(
                "SERVER: Received connection from {}. You can now send/receive messages".format(self.addr))
            ...
            self._AppendLog(
                "SERVER: Accepting connection failed: {}".format(str(e)))
            return False
    ...
    def _ReceiveMessages(self):
        while True:
            try:
                ...
                print("Received text is:", cipher_text)
                ...
                if not self.prtcl.GetIsSecure() and not self.prtcl.IsMessagePartOfProtocol(decoded_message):
                    self._AppendMessage("Other: {}".format(
                    ...
                    if self.prtcl.IsMessagePartOfProtocol(decoded_message):
                        ...
                        try:
                            next_message = self.prtcl.ProcessReceivedProtocolMessage(
                                decoded_message)
                            if next_message:
                    ...
                    # Otherwise, decrypting and showing the message
                        ...
                        plain_text = self.prtcl.DecryptAndVerifyMessage(
                            decoded_message)
                        self._AppendMessage("Other: {}".format(plain_text))

            except Exception as e:
                self._AppendLog(
                    "RECEIVER_THREAD: Error receiving data: {}".format(str(e)))
                return False
    ...
    def _SendMessage(self, message):
        ...
        if self.prtcl.GetIsSecure():
            send_message = self.prtcl.EncryptAndProtectMessage(send_message)
        else:
            send_message = json.dumps({"message": send_message})

        self.conn.send(send_message.encode())
    ...
    def SendMessage(self):
        ...
        if text != "" and self.s is not None:
            try:
                ...
                self._AppendLog(
                    "SENDING_MESSAGE: Error sending data: {}".format(str(e)))

        else:
            messagebox.showerror(
                "Networking", "Either the message is empty or the connection is not established.")
