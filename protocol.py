...
class Protocol:
    ...
    def GetProtocolInitResponseMessage(self, message):
        ...
        payload = self.EncryptAndProtectMessage(json.dumps(
            {"name": self._name, "nonce": message["nonce"], "dh": dh_val}), MsgType.INIT_REPLY, False, nonce)
    ...
    def GetProtocolEndMessage(self, message):
        ...
        dh_val = pow(self.g, self.private_key, self.p)
        self.SetSessionKey(
            pow(decoded_payload["dh"], self.private_key, self.p))
        payload = self.EncryptAndProtectMessage(json.dumps(
            {"name": self._name, "nonce": message["nonce"], "dh": dh_val}), MsgType.END, False)
        return payload
    ...
    def _verifyMsg(self, msg_type, decrypted):
        ...
        if msg_type == MsgType.INIT_REPLY or msg_type == MsgType.END:
            ...
            is_valid_reply_nonce = type(
                decrypted["nonce"]) == int and decrypted["nonce"] == self._sent_nonce
            is_valid_dh = type(decrypted["dh"]) == int

            validations.extend(
                [is_valid_reply_name, is_valid_reply_nonce, is_valid_dh])
    ...
    def EncryptAndProtectMessage(self, plain_text, type=MsgType.GENERAL, use_session_key=True, nonce=None):
        ...
        cipher_text = {"encrypted": b64encode(encoded_message).decode('utf-8'), "aes_nonce": b64encode(
            cipher.nonce).decode('utf-8'), "tag": b64encode(tag).decode('utf-8'), "type": type}
